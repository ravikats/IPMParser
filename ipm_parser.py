...
def write_csv(csv_file, results, compliance, file_violations, errors_only=False):
    """Write one row per record with key fields and compliance results.

    Parameters
    ----------
    csv_file : file object
        Opened for writing (text mode, newline="").
    results : list[dict]
        Per-record result dicts, as returned by ``process_file``.
    compliance : RulesEngine
        Used to enumerate rule ids for the development-mode per-rule
        PASS/FAIL columns.
    file_violations : list[ComplianceViolation]
        File-level compliance violations, appended as extra rows at the
        end of the CSV in both modes.
    errors_only : bool, optional (default False)
        - False (default / "development mode"): output is IDENTICAL to
          the original implementation -- same columns, same order, same
          content, for every record.
        - True ("production mode"): writes a compact CSV containing only
          record, mti, de2, de3, de4, de22, de37, de41, de42, errors,
          compliance -- and only for records that have parsing errors
          and/or compliance violations. Fully clean records are skipped.
          File-level violations are still appended at the end, exactly
          as in development mode.
    """
    if errors_only:
        # --- Production mode: compact, errors/violations-only CSV ---
        _write_csv_production(csv_file, results, file_violations)
        return

    # --- Development mode: unchanged from the original implementation ---
...
def _write_csv_production(csv_file, results, file_violations):
    """Write the compact "production" CSV (errors/violations only).

    Helper for ``write_csv(..., errors_only=True)``. Kept separate so the
    original development-mode code path in ``write_csv`` is left
    completely untouched.

    Columns: record, mti, de2, de3, de4, de22, de37, de41, de42, errors,
    compliance.

    Only records that have at least one parsing error and/or at least
    one compliance violation are written -- i.e. every record for which
    ``result["errors"]`` or ``result["compliance"]`` is truthy. Records
    with neither are skipped entirely to keep the file focused on
    problems that need operational attention.

    File-level compliance violations are still appended as trailing rows,
    exactly as in development mode.
    """
    header = [
        "record", "mti", "de3", "de4", "de22", "de37", "de41",
        "de42", "errors", "compliance",
    ]
    ...
    for result in results:
        # Skip records with no parsing errors AND no compliance
        # violations -- production mode only surfaces problem records.
        if not result.get("errors") and not result.get("compliance"):
            continue
